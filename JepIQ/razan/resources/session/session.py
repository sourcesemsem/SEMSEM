...
def get_api_id_and_hash():
    print("rickthon session pyrogram")
    print("\n---------------------------\n")
    try:
...
def telethon_session():
    ...
    try:
        with TelegramClient(StringSession(), API_ID, API_HASH) as jmthon:
            print("Generating a string session for •RICKTHON•")
            try:
               rickthon.send_message(
                    "me",
                    f"**هذا هو كود التيرمكس الخاص بك**:\n\n`{rickthon.session.save()}`\n\n**لا تشارك هذا الكود مع اي مستخدم !**\n@rickthon",
                )
                print(
        ...
        print(er)
        print("If you think It as a Bug, Report to @rickthon_group\n\n")
...
def pyro_session():
    ...
    with Client(":memory:", api_id=API_ID, api_hash=API_HASH) as pyro:
        ...
        pyro.send_message(
            "me",
            f"`{ss}`\n\n**الكود الذي في الاعلى هو كود البايروجرام لا تشاركه مع اي احد**",
        )
